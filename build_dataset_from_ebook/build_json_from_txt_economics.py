#!/usr/bin/env python3
"""Create a dataset from "The Wealth of Nations" text."""
...
CHAPTER_LINE_PATTERN = re.compile(r"^\s*CHAPTER\s+([IVXLCDM]+|\d+)\.\s*$", re.IGNORECASE)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Split wealth_of_nations.txt into chapter blocks.")
    parser.add_argument("--input", type=Path, default=Path("build_dataset_from_ebook/src_book/wealth_of_nations.txt"), help="Path to the source text file.")
    parser.add_argument("--output", type=Path, default=Path("build_dataset_from_ebook/src_book/wealth_of_nations_dataset.json"), help="Destination JSON file for the generated dataset.")
    parser.add_argument("--book_name", default="wealth_of_nations", help='Book identifier stored under the "book_name" field.')
...
def extract_chapters(text: str) -> list[tuple[str, str]]:
    ...
    idx = 0
    while idx < total:
        match = CHAPTER_LINE_PATTERN.match(lines[idx])
        if not match:
            idx += 1
            continue
        ...
        start_idx = idx
        while idx < total and not CHAPTER_LINE_PATTERN.match(lines[idx]):
            idx += 1
        body = "\n".join(lines[start_idx:idx]).strip()
        if not body:
            continue
        page_id = str(len(chapters) + 1)
...
def chunk_sentences_by_word_count(sentences: list[str], chunk_count: int) -> list[str]:
    ...
    for idx, (sentence, words) in enumerate(zip(sentences, word_counts)):
        ...
        should_split = (
            remaining_chunks > 0 and remaining_sentences > remaining_chunks and current_words >= target_words
        )
...
def build_dataset(chapters: list[tuple[str, str]], book_name: str, blocks_per_unit: int) -> list[dict]:
    dataset: list[dict] = []
    if not chapters:
        return dataset

    reference_idx = 1 if len(chapters) > 1 else 0
    reference_sentences = split_sentences(normalize_paragraph(chapters[reference_idx][1]))
    ...
    for _, raw_body in chapters:
        ...
        ratio = chapter_words / reference_words if reference_words else 1
        subchapter_count = int(ratio) if ratio >= 1 else 1
        ...
        for sub_text in subchapters:
            ...
            if not sub_sentences:
                continue
            blocks = chunk_sentences(sub_sentences, blocks_per_unit)
            record = {"book_name": book_name, "page_id": str(page_counter)}
...
def main() -> None:
    ...
    dataset = build_dataset(chapters, args.book_name, args.blocks)

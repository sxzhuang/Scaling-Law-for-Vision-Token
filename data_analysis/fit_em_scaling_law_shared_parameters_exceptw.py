"""Fit EM gating model across resolutions with per-resolution w0."""
...
DEFAULT_OUTPUT_DIR = Path("data_analysis/em_fiting_res/shared_exceptw")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Fit EM mixture scaling law with shared parameters and per-resolution w0.")
    ...
    logger = logging.getLogger("fit_em_scaling_law_shared_parameters_exceptw")
    ...
    var1 = (em_result.alpha1 * em_result.beta1) / (
        ((em_result.alpha1 + em_result.beta1) ** 2) * (em_result.alpha1 + em_result.beta1 + 1.0)
    )
    var0 = (em_result.alpha0 * em_result.beta0) / (
        ((em_result.alpha0 + em_result.beta0) ** 2) * (em_result.alpha0 + em_result.beta0 + 1.0)
    )
